#!/usr/bin/env python3
"""
ViT-B/32 training on CIFAR-100 with DDP and performance optimizations
    ...
    def __init__(self, num_classes=100, pretrained=True, drop_path_rate=0.1, use_checkpoint=True, 
        ...
        if pretrained:
            ...
            if npz_path and os.path.exists(npz_path):
                if is_main_process():
                    print(f"Loading ViT-B/32 from Google's official .npz weights...")
            ...
            if not loaded_npz:
                if is_main_process():
                    print("Loading ViT-B/32 with pretrained weights from timm...")
                ...
                try:
                    # 尝试 ImageNet-21k 预训练
                    self.model = timm.create_model(
                        'vit_base_patch32_224.augreg_in21k_ft_in1k',  # IN21K 预训练 + IN1K 微调
                        pretrained=True,
                        num_classes=num_classes,
                        drop_path_rate=drop_path_rate,
                        img_size=224
                ...
                if is_main_process():
                    print(f"✓ Loaded pretrained weights (ViT-B/32)")
            ...
            if is_main_process():
                print("Creating ViT-B/32 from scratch (no pretrained weights)...")
            
            self.model = timm.create_model(
                'vit_base_patch32_224',  # 使用标准 ViT-B/32 架构
                pretrained=False,
                num_classes=num_classes,
                drop_path_rate=drop_path_rate,
                img_size=224
...
def train_worker(rank, world_size, args):
    ...
    # Data transforms
    # Using 224x224 input (standard ViT-B/32 resolution)
    ...
    if is_main_process():
        ...
        with open(os.path.join(args.output_dir, "results_summary.txt"), "w") as f:
            f.write(f"ViT-B/32 on CIFAR-100 (DDP Optimized)\n")
            f.write(f"=" * 50 + "\n\n")
            f.write(f"Training Configuration:\n")
            f.write(f"  - Model: ViT-B/32 (pretrained)\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train ViT-B/32 on CIFAR-100 with DDP and optimizations')
