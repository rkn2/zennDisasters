#!/usr/bin/env python3
"""
ViT-B/32 CoupledModel training on CIFAR-10 with DDP and performance optimizations
...
# Model Definitions
# ============================================================================
class ViT_B_32_CIFAR(nn.Module):
    """ViT-B/32 for CIFAR-10 with optional pretrained weights"""
    def __init__(self, num_classes=10, pretrained=True, drop_path_rate=0.1, npz_path=None):
        ...
        if pretrained and npz_path is not None:
            ...
            if os.path.exists(npz_path):
                ...
                if success:
                    if is_main_process():
                        print(f"✓ Loaded ViT-B/32 with local pretrained weights from {npz_path}")
                else:
                    if is_main_process():
                    ...
                    if is_main_process():
                        print(f"✓ Loaded ViT-B/32 with timm pretrained weights")
            else:
                if is_main_process():
            ...
            try:
                ...
                if is_main_process():
                    print(f"✓ Loaded ViT-B/32 with ImageNet-21k pretrained weights")
            except Exception as e:
                if is_main_process():
                ...
                if is_main_process():
                    print(f"✓ Loaded ViT-B/32 with ImageNet-1k pretrained weights")
        else:
            # 从头训练
            self.model = timm.create_model(
                'vit_base_patch32_224',
            ...
            if is_main_process():
                print(f"✓ Created ViT-B/32 from scratch")
...
class CoupledModel(nn.Module):
    def __init__(self, width=224, num_classes=10, kb=1.0, in_channels=3, pretrained=True, drop_path_rate=0.1, rank=0, npz_path=None):
        ...
        # 使用 ViT-B/32 替代 ResNet18
        self.E_net = ViT_B_32_CIFAR(num_classes=num_classes, pretrained=pretrained, 
                                     drop_path_rate=drop_path_rate, npz_path=npz_path).to(self.device_E)
        self.S_net = ViT_B_32_CIFAR(num_classes=num_classes, pretrained=pretrained, 
...
def train_worker(rank, world_size, args):
    ...
    if is_main_process():
        print(f"\n{'='*80}")
        print(f"ViT-B/32 CoupledModel DDP Training")
        print(f"{'='*80}")
        print(f"Using {world_size} GPU(s) with DDP")
        for i in range(world_size):
            print(f"  GPU {i}: {torch.cuda.get_device_name(i)}")
        print(f"\nConfiguration:")
        print(f"  Model: ViT-B/32 (Base, patch size 32)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='ViT-B/32 CoupledModel Training with DDP')
    ...
    parser.add_argument('--npz-path', type=str, default='./pretrained_models/ViT-B_32.npz', 
