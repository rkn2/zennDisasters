#!/usr/bin/env python3
"""
ViT-L/32 CoupledModel training on CIFAR-10 with DDP and performance optimizations
...
def setup_ddp(rank, world_size):
    ...
    # Set MASTER_PORT if not already set (find a free port)
    if 'MASTER_PORT' not in os.environ:
        if rank == 0:
            # Only main process finds the port and it will be shared via file
            port = find_free_port()
            os.environ['MASTER_PORT'] = str(port)
            # Save port to a temporary file for other processes
            with open('/tmp/ddp_port.txt', 'w') as f:
                f.write(str(port))
        else:
            # Other processes wait and read the port
            import time
            max_wait = 30
            waited = 0
            while not os.path.exists('/tmp/ddp_port.txt') and waited < max_wait:
                time.sleep(0.1)
                waited += 0.1
            if os.path.exists('/tmp/ddp_port.txt'):
                with open('/tmp/ddp_port.txt', 'r') as f:
                    port = f.read().strip()
                os.environ['MASTER_PORT'] = port
            else:
                raise RuntimeError("Could not find port file")
...
def cleanup_ddp():
    ...
    # Clean up port file (only main process)
    if is_main_process() and os.path.exists('/tmp/ddp_port.txt'):
        try:
            os.remove('/tmp/ddp_port.txt')
        except:
            pass
...
# Model Definitions
# ============================================================================
class ViT_L_32_CIFAR(nn.Module):
    """ViT-L/32 for CIFAR-10 with optional pretrained weights"""
    def __init__(self, num_classes=10, pretrained=True, drop_path_rate=0.1, npz_path=None):
        super().__init__()
        ...
        if pretrained and npz_path is not None:
            ...
            if os.path.exists(npz_path):
                ...
                if success:
                    if is_main_process():
                        print(f"✓ Loaded ViT-L/32 with local pretrained weights from {npz_path}")
                else:
                    if is_main_process():
                    ...
                    if is_main_process():
                        print(f"✓ Loaded ViT-L/32 with timm pretrained weights")
            else:
                if is_main_process():
            ...
            try:
                self.model = timm.create_model(
                    'vit_large_patch32_224.orig_in21k_ft_in1k',
                    pretrained=True,
                    num_classes=num_classes,
                    drop_path_rate=drop_path_rate,
                    img_size=224
                ...
                if is_main_process():
                    print(f"✓ Loaded ViT-L/32 with ImageNet-21k pretrained weights")
            except Exception as e:
                if is_main_process():
                ...
                if is_main_process():
                    print(f"✓ Loaded ViT-L/32 with ImageNet-1k pretrained weights")
        else:
            # 从头训练
            self.model = timm.create_model(
                'vit_large_patch32_224',
            ...
            if is_main_process():
                print(f"✓ Created ViT-L/32 from scratch")
...
class CoupledModel(nn.Module):
    def __init__(self, width=224, num_classes=10, kb=1.0, in_channels=3, pretrained=True, drop_path_rate=0.1, rank=0, npz_path=None):
        super().__init__()
        self.C  = num_classes
        self.kb = kb
        self.rank = rank
        ...
        # 使用 ViT-L/32 替代 ResNet18
        self.E_net = ViT_L_32_CIFAR(num_classes=num_classes, pretrained=pretrained, 
                                     drop_path_rate=drop_path_rate, npz_path=npz_path).to(self.device_E)
        self.S_net = ViT_L_32_CIFAR(num_classes=num_classes, pretrained=pretrained, 
...
def train_worker(rank, world_size, args):
    ...
    if is_main_process():
        print(f"\n{'='*80}")
        print(f"ViT-L/32 CoupledModel DDP Training")
        print(f"{'='*80}")
        print(f"Using {world_size} GPU(s) with DDP")
        for i in range(world_size):
            print(f"  GPU {i}: {torch.cuda.get_device_name(i)}")
        print(f"\nConfiguration:")
        print(f"  Model: ViT-L/32 (Large, patch size 32)")
    ...
    # Download dataset only on main process to avoid conflicts
    if is_main_process():
        CIFAR10(root=args.data_dir, train=True, download=True, transform=train_transform)
        CIFAR10(root=args.data_dir, train=False, download=True, transform=eval_transform)
    ...
    train_dataset = CIFAR10(root=args.data_dir, train=True, download=False, transform=train_transform)
    test_dataset = CIFAR10(root=args.data_dir, train=False, download=False, transform=eval_transform)
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (x, y) in enumerate(train_loader):
            ...
            y_onehot = F.one_hot(y, num_classes=10).float().to(device, non_blocking=True)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='ViT-L/32 CoupledModel Training with DDP')
    ...
    parser.add_argument('--npz-path', type=str, default='./pretrained_models/ViT-L_32.npz', 
