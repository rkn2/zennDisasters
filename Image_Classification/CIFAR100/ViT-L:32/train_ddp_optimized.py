#!/usr/bin/env python3
"""
ViT-L/32 training on CIFAR-100 with DDP and performance optimizations
...
# ============================================================================
def interpolate_pos_embed(pos_embed, orig_size=7, new_size=7):
    """
    Interpolate position embeddings for different image sizes
    
    Args:
        pos_embed: (1, N+1, dim) where N = orig_size * orig_size
        orig_size: original grid size (7 for 224x224 with patch 32)
        new_size: new grid size (7 for 224x224 with patch 32)
...
def load_npz_weights(model, npz_path, num_classes=100):
    ...
    try:
        ...
        # Note: We skip loading the classifier head since we're training for CIFAR-100
        # The head will be randomly initialized for num_classes=100
...
class ViT_L_32_CIFAR(nn.Module):
    def __init__(self, num_classes=100, pretrained=True, drop_path_rate=0.1, use_checkpoint=True, 
                 npz_path=None):
        super().__init__()
        ...
        if pretrained:
            ...
            if npz_path and os.path.exists(npz_path):
                if is_main_process():
                    print(f"Loading ViT-L/32 from Google's official .npz weights...")
            ...
            if not loaded_npz:
                if is_main_process():
                    print("Loading ViT-L/32 with pretrained weights from timm...")
                ...
                try:
                    # 尝试 ImageNet-21k 预训练
                    self.model = timm.create_model(
                        'vit_large_patch32_224.orig_in21k_ft_in1k',  # IN21K 预训练 + IN1K 微调
                        pretrained=True,
                        num_classes=num_classes,
                        drop_path_rate=drop_path_rate,
                        img_size=224
                    ...
                    model_name = "vit_large_patch32_224.orig_in21k_ft_in1k"
                except Exception as e:
                    if is_main_process():
                ...
                if is_main_process():
                    print(f"✓ Loaded pretrained weights (ViT-L/32)")
            ...
            if is_main_process():
                print("Creating ViT-L/32 from scratch (no pretrained weights)...")
            
            self.model = timm.create_model(
                'vit_large_patch32_224',  # 使用标准 ViT-L/32 架构
                pretrained=False,
                num_classes=num_classes,
                drop_path_rate=drop_path_rate,
                img_size=224
...
def plot_training_curves(train_losses, test_accuracies, learning_rates, epoch_times, save_path):
    ...
    info_text += f"Dataset: CIFAR-100\n"
    info_text += f"Model: ViT-L/32\n\n"
    info_text += f"Results:\n"
...
def train_worker(rank, world_size, args):
    ...
    # Data transforms
    # Using 224x224 input (standard ViT-L/32 resolution)
    ...
    # Then synchronize before other processes load the data
    if is_main_process():
    ...
    # Now all processes can load the dataset (download=False for non-main processes)
    ...
    if is_main_process():
        ...
        with open(os.path.join(args.output_dir, "results_summary.txt"), "w") as f:
            f.write(f"ViT-L/32 on CIFAR-100 (DDP Optimized)\n")
            f.write(f"=" * 50 + "\n\n")
            f.write(f"Training Configuration:\n")
            f.write(f"  - Model: ViT-L/32 (pretrained)\n")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train ViT-L/32 on CIFAR-100 with DDP and optimizations')
